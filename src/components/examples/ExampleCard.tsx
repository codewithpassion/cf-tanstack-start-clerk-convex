 ...
 * Shows title, content preview, notes, file count, and action buttons.
 */
export function ExampleCard({ example, onEdit, onDelete }: ExampleCardProps) {
	const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);

	// Fetch files for this example to show count
	...
	return (
		<>
			<div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-6 hover:border-slate-300 dark:hover:border-slate-600 transition-colors">
				<div className="flex items-start justify-between gap-4">
					<div className="flex-1 min-w-0">
						...
						{example.notes && (
							<div className="mb-4 p-3 bg-yellow-50 dark:bg-yellow-950/30 border-l-4 border-yellow-400 dark:border-yellow-600 rounded">
			...
			<ConfirmDialog
				...
				title="Delete Example"
				message={`Are you sure you want to delete "${example.title}"? This action cannot be undone. All attached files will also be deleted.`}
				confirmLabel="Delete"
